...
int nova_alloc_block_free_lists(struct super_block *sb)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		init_rwsem_cst(&free_list->s_lock);
		free_list->index = i;
	}
	...
	int num, unsigned short btype, int log_page)
{
	...
	int new_node_used = 0;
	int ret;
	timing_t free_time,jh_free_time;
	...
	NOVA_CLEAR_TIMING_STATS_NOW(jh_free_t, nova_get_cpuid(sb));
	NOVA_START_TIMING(jh_free_t, jh_free_time);
	down_write_cst(&free_list->s_lock);
...
out:
	...
	up_write_cst(&free_list->s_lock);
	NOVA_END_TIMING(jh_free_t, jh_free_time);
	nova_print_timing_stats_now_free(nova_get_cpuid(sb));
	...
	enum alloc_type atype, int cpuid, enum nova_alloc_direction from_tail)
{
	...
	long ret_blocks = 0;
	int retried = 0;
	timing_t alloc_time,jh_alloc_time;
...
retry:
	free_list = nova_get_free_list(sb, cpuid);
	//spin_lock(&free_list->s_lock);
	NOVA_CLEAR_TIMING_STATS_NOW(jh_alloc_t, nova_get_cpuid(sb));
	NOVA_START_TIMING(jh_alloc_t, jh_alloc_time);
	down_write_cst(&free_list->s_lock);
...
alloc:
	...
	//spin_unlock(&free_list->s_lock);
	up_write_cst(&free_list->s_lock);
	NOVA_END_TIMING(jh_alloc_t, jh_alloc_time);
	nova_print_timing_stats_now_alloc(nova_get_cpuid(sb));
