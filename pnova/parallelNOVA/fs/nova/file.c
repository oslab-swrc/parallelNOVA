...
void nova_segment_write_lock(struct nova_inode_info_header *sih,
		unsigned long long start, unsigned long long size)
{
	atomic_t* segment_rwsem = sih->segment_rwsem, *dbg_ptr;
	...
	for( i=start ; i<start+size; i++){
		//dbg_ptr = &segment_rwsem[i];
		trials = 0;
		while(true){
			trials++;
			//nova_dbg("before write lock:\n");
			//printBits(sizeof(*dbg_ptr), dbg_ptr);
...
out:
 ...
 void nova_segment_write_unlock(struct nova_inode_info_header *sih,
                unsigned long long start, unsigned long long size)
{
	atomic_t* segment_rwsem = sih->segment_rwsem, *dbg_ptr;
	int jh,i;
	...
	for( i=start ; i<start+size; i++){
		...
		while(1){
			...
			if (trials % 100000 == 0 ) {
				if ( trials == 1000000000 ){
					nova_dbg("seg w unlock failed \n");
					//nova_dbg("seg w unlock failed after %10ld times, start: %10ld, size: %10ld segs\n", trials, start2, size2);
					break;
				}
			}
		}
...
void nova_segment_read_lock(struct nova_inode_info_header *sih,
                unsigned long long start, unsigned long long size)
{
	int i;
	atomic_t* segment_rwsem = sih->segment_rwsem;
	unsigned long trials ,jh;
	int rwtry = 0;
	unsigned int wlock;
	wlock = (unsigned int)1 << 31;
	...
	for( i=start ; i<start+size; i++){
		...
		while(1){
			...
			smp_mb__after_atomic();		
			//nova_dbg("unlokc jh : %u",jh);
			if(jh != 0)
				break;
			if (trials % 100000 == 0 ) {
				if ( trials == 10000000 ){
					nova_dbg("seg r lock failed\n");
					//nova_dbg("seg r lock failed after %10ld times, start: %10ld, size: %10ld segs\n", trials, start2, size2);
					break;
				}
			}
		}
...
void nova_segment_read_unlock(struct nova_inode_info_header *sih,
                unsigned long long start, unsigned long long size)
{
	int i;
	atomic_t* segment_rwsem = sih->segment_rwsem;
		...
		size_t len, loff_t *ppos)
{
	...
	start_blk = pos >> sb->s_blocksize_bits;
	end_blk = (pos + len - 1) >> sb->s_blocksize_bits;

 	start_seg = pos >> SEGMENT_SIZE_BITS;
	end_seg = (pos + len - 1) >> SEGMENT_SIZE_BITS;
	
	nova_segment_read_lock(sih, start_seg, end_seg - start_seg + 1);
...
out:
	*ppos = pos + copied;
	...
	nova_segment_read_unlock(sih, start_seg, end_seg - start_seg + 1);
		...
		const char __user *buf,	size_t len, loff_t *ppos)
{
	...
	int i, cpuid;
	timing_t locktime,locktime_size;
	size_t count2;	
	unsigned long start_seg, end_seg;
	struct pc_log_pointer *log_pointer;
...
*/
	...
	log_pointer  = sih->pc_log_pointer[cpuid];
...
*/
	...
	if(sih->pc_log_pointer[cpuid] == 0) /* per-core log pointer is not allocated yet */
		update.tail = 0;
	else
		update.tail = sih->pc_log_pointer[cpuid]->tail;	
...
/*		TODO: for scalable test*/
/*
		down_write_cst(&sih->size_rwsem_cst);
		...
		up_write_cst(&sih->size_rwsem_cst);
*/
		//queued_spin_unlock(&sih->size_lock);

		NOVA_END_TIMING(jh_size_t, locktime_size);
		//nova_print_timing_stats_now_size(nova_get_cpuid(sb));
...
/*	TODO: for scalable test */
	...
	/* scalable bottleneck occurs here! */
/*
	down_write_cst(&sih->block_rwsem_cst);

	sih->i_blocks += (total_blocks << (data_bits - sb->s_blocksize_bits));
	inode->i_blocks = sih->i_blocks;
	sih->trans_id++;

	up_write_cst(&sih->block_rwsem_cst);
*/

	sih->pc_log_pointer[cpuid]->tail = update.tail;
...
*/
	...
	ret = nova_reassign_file_tree_parallel(sb, sih, begin_tail, cpuid);
	if (ret)
		goto out;
...
out:
	...
	//nova_segment_write_unlock(sih, start_seg, end_seg - start_seg + 1);
	return ret;
}
...
//	nova_print_timing_stats(inode->i_sb);
