...
static int nova_gc_assign_file_entry_parallel(struct super_block *sb,
	struct nova_inode_info_header *sih,
	struct nova_file_write_entry *old_entry,
	struct nova_file_write_entry *new_entry)
{
	...
	struct pc_log_pointer *point = sih->pc_log_pointer[cpuid];
	...
	for (i = 0; i < num; i++) {
		...
		if (pentry) {
			temp = radix_tree_deref_slot(pentry);
			if (temp == old_entry)
				radix_tree_replace_slot(&point->tree, pentry,
							new_entry);
		}
	}
	...
	u64 curr_p, u64 new_curr)
{
	...
	case FILE_WRITE:
		...
		/*TODO*/
		//ret = nova_gc_assign_file_entry_parallel(sb, sih, old_entry, new_entry);
		break;
	case DIR_LOG:
		new_addr = (void *)nova_get_block(sb, new_curr);
...
static unsigned long nova_inode_log_thorough_gc_parallel(struct super_block *sb,
	struct nova_inode *pi, struct nova_inode_info_header *sih,
	unsigned long blocks, unsigned long checked_pages)
{
	struct nova_inode_log_page *curr_page = NULL;
	...
	//curr_p = sih->log_head;
	curr_p = sih->pc_log_pointer[cpuid]->head;
	old_curr_p = curr_p;
	//old_head = sih->log_head;
	old_head = sih->pc_log_pointer[cpuid]->head;
	//nova_dbg_verbose("Log head 0x%llx, tail 0x%llx\n",
	//			curr_p, sih->log_tail);
	if (curr_p == 0 && sih->pc_log_pointer[cpuid]->tail == 0)
		goto out;

	if (curr_p >> PAGE_SHIFT == sih->pc_log_pointer[cpuid]->tail >> PAGE_SHIFT)
		goto out;
	...
	new_curr = new_head;
	while (curr_p != sih->pc_log_pointer[cpuid]->tail) {
		old_curr_p = curr_p;
		if (goto_next_page(sb, curr_p))
			curr_p = next_log_page(sb, curr_p);
		...
		if (!ret) {
			extended = 0;
			new_curr = nova_get_append_head_parallel(sb, pi, sih,
						new_curr, length, MAIN_LOG,
						1, &extended);
			if (extended)
				blocks++;
	...
	/* Step 1: Link new log to the tail block */
	tail_block = BLOCK_OFF(sih->pc_log_pointer[cpuid]->tail);
	curr_page = (struct nova_inode_log_page *)nova_get_block(sb,
							BLOCK_OFF(new_curr));
	next = next_log_page(sb, new_curr);
	if (next > 0)
	...
	nova_memlock_inode(sb, pi);
	nova_flush_buffer(pi, sizeof(struct nova_inode), 1);
	sih->pc_log_pointer[cpuid]->head = new_head;
	...
	sih->pc_log_pointer[cpuid]->log_pages = sih->pc_log_pointer[cpuid]->log_pages + blocks - checked_pages;
	NOVA_STATS_ADD(thorough_gc_pages, checked_pages - blocks);
	NOVA_STATS_ADD(thorough_checked_pages, checked_pages);
out:
	NOVA_END_TIMING(thorough_gc_t, gc_time);
...
int nova_inode_log_fast_gc_parallel(struct super_block *sb,
	struct nova_inode *pi, struct nova_inode_info_header *sih,
	u64 curr_tail, u64 new_block, u64 alter_new_block,
	int num_pages, int force_thorough, int cpu_num)
{
	struct nova_inode *alter_pi;
	u64 curr, next, possible_head = 0;
	u64 alter_curr, alter_next = 0, alter_possible_head = 0;
	...
	NOVA_START_TIMING(fast_gc_t, gc_time);
	//curr = sih->log_head;
	curr = sih->pc_log_pointer[cpu_num]->head;
	alter_curr = sih->alter_log_head;
	sih->valid_entries = 0;
	sih->num_entries = 0;
	...
	nova_dbgv("%s: log head 0x%llx, tail 0x%llx\n",
				__func__, curr, curr_tail);
	while (1) {
		if (curr >> PAGE_SHIFT == sih->pc_log_pointer[cpu_num]->tail >> PAGE_SHIFT) {
			/* Don't recycle tail page */
			if (found_head == 0) {
				possible_head = cpu_to_le64(curr);
				alter_possible_head = cpu_to_le64(alter_curr);
		...
		if (curr_page_invalid(sb, pi, sih, curr)) {
			nova_dbg_verbose("curr page %p invalid\n", curr_page);
			if (curr == sih->pc_log_pointer[cpu_num]->head) {
				/* Free first page later */
				first_need_free = 1;
				last_page = curr_page;
				alter_last_page = alter_curr_page;
...
*/
	...
	nova_memlock_inode(sb, pi);
	*/
	sih->pc_log_pointer[cpu_num]->head = possible_head;
	sih->alter_log_head = alter_possible_head;
	nova_dbgv("%s: %d new head 0x%llx\n", __func__,
					found_head, possible_head);
	sih->pc_log_pointer[cpu_num]->log_pages += (num_pages - freed_pages) * num_logs;
	/* Don't update log tail pointer here */
	/*TODO: testing
	nova_flush_buffer(&pi->log_head, CACHELINE_SIZE, 1);
	*/
	...
	if (force_thorough || (blocks && blocks * 2 < checked_pages)) {
		nova_dbgv("Thorough GC for inode %lu: checked pages %lu, valid pages %lu\n",
				sih->ino,
				checked_pages, blocks);
		blocks = nova_inode_log_thorough_gc_parallel(sb, pi, sih,
							blocks, checked_pages);
		if (metadata_csum)
			nova_inode_alter_log_thorough_gc(sb, pi, sih,
							blocks, checked_pages);
