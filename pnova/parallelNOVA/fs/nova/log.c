...
static void nova_init_log_pointer(struct nova_inode_info_header *sih, int num)
{
	struct pc_log_pointer *new_node;
	new_node = kzalloc(sizeof(struct pc_log_pointer), GFP_KERNEL);
	INIT_RADIX_TREE(&new_node->tree,GFP_ATOMIC);
	new_node->node_num = num;
	new_node->head = new_node->tail = 0;
	new_node->log_pages = 0;
	sih->pc_log_pointer[num] = new_node;
}
	...
	unsigned int num_free, struct nova_inode_info_header *sih)
{
	...
	if (invalid) {
		...
		NOVA_CLEAR_TIMING_STATS_NOW(jh_invalid_t, nova_get_cpuid(sb));
		NOVA_START_TIMING(jh_invalid_t, jh_invalid);
		//down_write_cst(&sih->inval_rwsem_cst);
		nova_inc_page_invalid_entries(sb, addr);
        	//queued_spin_unlock(&sih->inval_lock);
		//up_write_cst(&sih->inval_rwsem_cst);
		NOVA_END_TIMING(jh_invalid_t, jh_invalid);
		//nova_print_timing_stats_now_invalid(nova_get_cpuid(sb));
	}
    ...
    u64 old_tail)
{
    ...
    //curr_p = old_tail;
	curr_p = nova_get_append_head_parallel(sb,pi,sih,tail,size,MAIN_LOG,0,&extended);
	...
	//NOVA_CLEAR_TIMING_STATS_NOW(jh_valid_t, nova_get_cpuid(sb));
	//NOVA_START_TIMING(jh_valid_t, jh_valid);

	//down_write_cst(&sih->entry_rwsem_cst);
	nova_inc_page_num_entries(sb, curr_p);
    	//queued_spin_unlock(&sih->entry_lock);
	//up_write_cst(&sih->entry_rwsem_cst);

	//NOVA_END_TIMING(jh_valid_t, jh_valid);
	//nova_print_timing_stats_now_valid(nova_get_cpuid(sb));
	...
	bool free)
{
	...
	timing_t assign_time;
	/*TODO: still doing..*/
	struct pc_log_pointer *point;
	int cpuid = nova_get_cpuid(sb);

	point  = sih->pc_log_pointer[cpuid];
	...
	for (i = 0; i < num; i++) {
		...
		pentry = radix_tree_lookup_slot(&sih->tree, curr_pgoff);
		//pentry = radix_tree_lookup_slot(&point->tree, curr_pgoff);
		if (pentry) {
			old_entry = radix_tree_deref_slot(pentry);
			if (old_entry != start_old_entry) {
				if (start_old_entry && free)
			...
			//radix_tree_replace_slot(&point->tree, pentry, entry);
			radix_tree_replace_slot(&sih->tree, pentry, entry);
		} else {
			//ret = radix_tree_insert(&point->tree, curr_pgoff, entry);
			ret = radix_tree_insert(&sih->tree, curr_pgoff, entry);
			if (ret) {
				nova_dbg("%s: ERROR %d\n", __func__, ret);
				goto out;
...
int nova_allocate_inode_log_pages_test(struct super_block *sb,
	struct nova_inode_info_header *sih, unsigned long num_pages,
	u64 *new_block, int cpuid, enum nova_alloc_direction from_tail)
{
	unsigned long new_inode_blocknr;
	...
	cpun0=nova_get_cpuid(sb)%15;
	cpun1=cpun0+15;
	allocated = nova_new_log_blocks(sb, sih, &new_inode_blocknr,
			num_pages, ALLOC_INIT_ZERO, cpun1, from_tail);
	...
	*new_block = nova_get_block_off(sb, first_blocknr,
						NOVA_BLOCK_TYPE_4K);
	nova_dbg("jh cpun0 : %d cpun1 : %d",cpun0,cpun1);
...
static int nova_initialize_inode_log_parallel(struct super_block *sb,
	struct nova_inode *pi, struct nova_inode_info_header *sih,
	int log_id,int cpu_num)
{
	u64 new_block;
	int allocated;

	nova_init_log_pointer(sih,cpu_num);
	...
	if (log_id == MAIN_LOG) {
		...
		nova_flush_buffer(&pi->log_tail, CACHELINE_SIZE, 0);
		pi->log_head = new_block;
		sih->pc_log_pointer[cpu_num]->head = sih->pc_log_pointer[cpu_num]->tail = new_block;
		sih->pc_log_pointer[cpu_num]->log_pages = 1;
		nova_flush_buffer(&pi->log_head, CACHELINE_SIZE, 1);
	} else {
		pi->alter_log_tail = new_block;
		nova_flush_buffer(&pi->alter_log_tail, CACHELINE_SIZE, 0);
		pi->alter_log_head = new_block;
		sih->alter_log_head = sih->alter_log_tail = new_block;
		sih->pc_log_pointer[cpu_num]->log_pages++;
		nova_flush_buffer(&pi->alter_log_head, CACHELINE_SIZE, 1);
	}
	nova_update_inode_checksum(pi);
	nova_memlock_inode(sb, pi);
...
static u64 nova_extend_inode_log_parallel(struct super_block *sb, struct nova_inode *pi,
	struct nova_inode_info_header *sih, u64 curr_p,int cpu_num)
{
	u64 new_block, alter_new_block = 0;
	int allocated;
	unsigned long num_pages;
	...
	if (curr_p == 0) {
		ret = nova_initialize_inode_log_parallel(sb, pi, sih, MAIN_LOG, cpu_num);
		if (ret)
			return 0;

		if (metadata_csum) {
			ret = nova_initialize_inode_log_parallel(sb, pi, sih, ALTER_LOG,cpu_num);
			if (ret)
				return 0;
		...
		return sih->pc_log_pointer[cpu_num]->head;
	}
	num_pages = sih->pc_log_pointer[cpu_num]->log_pages >= EXTEND_THRESHOLD ?
				EXTEND_THRESHOLD : sih->pc_log_pointer[cpu_num]->log_pages;
//	nova_dbg("Before append log pages:\n");
//	nova_print_inode_log_page(sb, inode);
	allocated = nova_allocate_inode_log_pages(sb, sih,
					num_pages, &new_block, ANY_CPU, 0);
	...
	nova_inode_log_fast_gc_parallel(sb, pi, sih, curr_p,
			       new_block, alter_new_block, allocated, 0, cpu_num);
...
{
	u64 curr_p;
	int cpu_num = nova_get_cpuid(sb);

	if (tail)
		curr_p = tail;
	else if (log_id == MAIN_LOG)
		if(sih->pc_log_pointer[cpu_num] == 0)
			curr_p = 0;
		else
			curr_p = sih->pc_log_pointer[cpu_num]->tail;
	else
		curr_p = sih->alter_log_tail;
...
//	nova_dbg("cpu %d, curr 0x%llx\n", cpu_num, curr_p);
	...
	if (curr_p == 0 || (is_last_entry(curr_p, size) &&
		...
		if (thorough_gc == 0) {
			curr_p = nova_extend_inode_log_parallel(sb, pi, sih, curr_p, cpu_num);
		} else {
			curr_p = nova_append_one_log_page(sb, sih, curr_p);
			/* For thorough GC */
			*extended = 1;
