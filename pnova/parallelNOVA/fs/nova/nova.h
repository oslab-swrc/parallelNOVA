...
/*For range lock*/
#define SEGMENT_MAX_NUMBER 1024*1024
...
/*
		nova_dbg("CPU ID: %d Good: Page 0x%llx has %u entries, %u invalid\n",
				nova_get_cpuid(sb), curr,
				curr_page->page_tail.num_entries,
				curr_page->page_tail.invalid_entries);
*/	
	nova_flush_buffer(&curr_page->page_tail,
...
int nova_reassign_file_tree_parallel(struct super_block *sb,
	struct nova_inode_info_header *sih, u64 begin_tail, int cpu_num);
