...
void nova_print_timing_stats_now_tail(int cpuid)
{
	if(measure_timing==1)
		nova_dbg("TAIL, CPU : %d Time : %llu", cpuid, per_cpu(Timingstats_percpu[ jh_tail_t ], cpuid));
}
void nova_print_timing_stats_now_size(int cpuid)
{
	if(measure_timing==1)
		nova_dbg("SIZE, CPU : %d Time : %llu", cpuid, per_cpu(Timingstats_percpu[ jh_size_t ], cpuid));
}
void nova_print_timing_stats_now_alloc(int cpuid)
{
	if(measure_timing==1)
		nova_dbg("ALLOC, CPU : %d Time : %llu", cpuid, per_cpu(Timingstats_percpu[ jh_alloc_t ], cpuid));
}
void nova_print_timing_stats_now_free(int cpuid)
{
	if(measure_timing==1)
		nova_dbg("FREE, CPU : %d Time : %llu", cpuid, per_cpu(Timingstats_percpu[ jh_free_t ], cpuid));
}
void nova_print_timing_stats_now_invalid(int cpuid)
{
	if(measure_timing==1)
		nova_dbg("INVALID, CPU : %d Time : %llu", cpuid, per_cpu(Timingstats_percpu[ jh_invalid_t ], cpuid));
}
void nova_print_timing_stats_now_valid(int cpuid)
{
	if(measure_timing==1)
		nova_dbg("VALID, CPU : %d Time : %llu", cpuid, per_cpu(Timingstats_percpu[ jh_valid_t ], cpuid));
}
